...
def mrpolygon(alp,sigma,mu=10,X_0=10,dt=0.001,nPoints=30):
    ...
    while t < 2*np.pi:
        bt = np.random.normal()
        X1.append(X1[-1] + alp*(mu - X1[-1])*dt + sigma*np.sqrt(dt)*bt)
        l = (dt**2 + (X1[-2]-X1[-1])**2)**0.5
        alpha = np.arccos((2*r[-1]**2 - dt**2)/float(2*r[-1]**2)) # phi_1
        beta = (np.pi - alpha)/float(2)
        #beta = numpy.arcsin(r[-1]*numpy.sin(alpha)/float(dt)) # phi_2
        if X1[-1] >= X1[-2]: #phi_3
            betap = np.pi - beta
        else:
            betap = beta
        psi = np.arcsin(dt*np.sin(betap)/float(l)) #phi_4
        phi = np.pi - betap - psi #phi_5
        rtp = l*np.sin(phi)/float(np.sin(betap))
        if alpha + a[-1] >= 2*np.pi:
            r.append(r[0])
            a.append(np.pi*2)
            ...
            if X1[-1] >= X1[-2]:
                ...
                if r[-1] - rtp <= 1:
                    ...
                    r.append(r[-1] - rtp)
            a.append(a[-1] + alpha)
            distance = (r[-2]**2 + r[-1]**2 - 2*r[-2]*r[-1]*np.cos(alpha))**0.5
            lengthCarte += l
            lengthPolar += distance
        times.append(t)
        t = a[-1]
...
def polarPolygon2cartesian(polarPolygon):
    ...
    for nr,point in enumerate(polarPolygon):
        polygon.append([polarPolygon[nr][1]*np.cos(polarPolygon[nr][0]),
                        polarPolygon[nr][1]*np.sin(polarPolygon[nr][0])])
    return polygon

def transportPolygonGeometry(polygon,angle,dx,dy,multx,multy):
    newPolygon = []
    for p in polygon:
        rotx = (p[0]*np.cos(angle) - p[1]*np.sin(angle))*multx
        roty = (p[0]*np.sin(angle) + p[1]*np.cos(angle))*multy
        transx = dx - rotx
        transy = dy - roty
        point = (transx,transy)
        newPolygon += [point]
...
def transportPolygon(polygon,center,pointa=0):
    if (center != 0) or (pointa != 0):
        ...
        if transx==0 and transy!=0:
            ...
            t = np.arctan(transy/float(transx))
